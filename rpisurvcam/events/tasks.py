...
def upload_motion_to_dropbox(motion_path, dropbox_path):
	...
	proc = subprocess.Popen([dropbox_uploader,'upload',video_files,dropbox_videos], stdout=subprocess.PIPE)
	...
	for item in (f.split('"') for f in files if 'DONE' in f):
		remote_file = item[3]
		proc = subprocess.Popen([dropbox_uploader,'share',remote_file], stdout=subprocess.PIPE)
