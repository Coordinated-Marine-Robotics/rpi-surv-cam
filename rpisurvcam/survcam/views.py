...
def index(request):
	return render_to_response('survcam/stream.html',{'camera' : Camera.objects.first(), 'stream_url' : get_motion_stream_url(request) })
...
def snapshot(request):
	...
	response['Content-Disposition'] = 'attachment; filename=%s' % path.basename(path.realpath(snapshot_file.name))
	return response	
